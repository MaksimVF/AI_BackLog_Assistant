...
def test_team_api():
    ...
    with app.app_context():
        ...
        with app.test_client() as client:
            ...
            # Add another team member
            response = client.post(f'/api/v1/team/members?org_id={org_id}')
            print(f"Add sixth team member response: {response.status_code}")
            print(f"Response data: {response.get_data(as_text=True)}")

            # Add another team member
            response = client.post(f'/api/v1/team/members?org_id={org_id}')
            print(f"Add seventh team member response: {response.status_code}")
            print(f"Response data: {response.get_data(as_text=True)}")

            # Add another team member
            response = client.post(f'/api/v1/team/members?org_id={org_id}')
            print(f"Add eighth team member response: {response.status_code}")
            print(f"Response data: {response.get_data(as_text=True)}")

            # Add another team member
            response = client.post(f'/api/v1/team/members?org_id={org_id}')
            print(f"Add ninth team member response: {response.status_code}")
            print(f"Response data: {response.get_data(as_text=True)}")

            # Try to add too many team members (should fail)
            response = client.post(f'/api/v1/team/members?org_id={org_id}')
            print(f"Add tenth team member response: {response.status_code}")
            print(f"Response data: {response.get_data(as_text=True)}")

            # Try to add too many team members (should fail)
            response = client.post(f'/api/v1/team/members?org_id={org_id}')
            print(f"Add eleventh team member response: {response.status_code}")
            print(f"Response data: {response.get_data(as_text=True)}")
