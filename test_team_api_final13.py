...
def test_team_api():
    ...
    with app.app_context():
        ...
        org_balance = OrganizationBalance(
            ...
            balance_rub=20.0,  # Insufficient balance for the next member
        ...
        with app.test_client() as client:
            ...
            # Try to remove a team member (should succeed)
            ...
            # Try to add another team member (should succeed)
            response = client.post(f'/api/v1/team/members?org_id={org_id}')
            print(f"Add team member after removal response: {response.status_code}")
            print(f"Response data: {response.get_data(as_text=True)}")
